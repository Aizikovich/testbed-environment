...
class CellDatabase(DATABASE):
    ...
    def config(self):
        ...
        if 'cell_features' in cfg.sections():
            self.thpt_dl = cfg.get('cell_features', 'thpt_dl')
            self.thpt_ul = cfg.get('cell_features', 'thpt_ul')
            self.nb_cells = cfg.get('cell_features', 'nb_cells')
            self.serv_cell = cfg.get('cell_features', 'serv_cell')
            self.ue_id = cfg.get('cell_features', 'ue_id')
            self.cell_id = cfg.get('cell_features', 'cell_id')
    ...
    def assign_timestamp(self, data):
        ...
        for timestamp in steps:
            d = df[df['measTimeStampRf'] == timestamp]
            d.index = pd.date_range(start=datetime.datetime.now(), freq='1ms', periods=len(d))
            self.client.write_points(d, self.cell_meas)
            time.sleep(0.4)
