...
class UserEquipment:
    ...
    def __init__(self, env, ue_id, initial_data_rate, starting_position, speed=0, direction=0, random=False,
                 _lambda_c=None, _lambda_d=None):
        self.ue_id = ue_id
    ...
    def random_move(self):
        ...
        if val == 1:
            if (self.current_position[0] + size) > 0 and (self.current_position[0] + size) < x_lim:
                ...
                self.current_position[0] + size, self.current_position[1], self.current_position[2])
        elif val == 2:
            if (self.current_position[0] - size) > 0 and (self.current_position[0] - size) < x_lim:
                self.current_position = (
                self.current_position[0] - size, self.current_position[1], self.current_position[2])
            ...
            if (self.current_position[1] + size) > 0 and (self.current_position[1] + size) < y_lim:
                self.current_position = (
                self.current_position[0], self.current_position[1] + size, self.current_position[2])
        else:
            if (self.current_position[1] - size) > 0 and (self.current_position[1] - size) < y_lim:
                self.current_position = (
                self.current_position[0], self.current_position[1] - size, self.current_position[2])
    ...
    def line_move(self):
        ...
        if ((self.current_position[0] != 0 and math.tan(math.radians(self.direction)) == (self.current_position[1]) / (
        self.current_position[0])) or (self.direction == 270)):
        ...
        elif ((x_lim - self.current_position[0] != 0 and math.tan(math.radians(self.direction)) == (
                y_lim - self.current_position[1]) / (x_lim - self.current_position[0])) or (self.direction == 90)):
        ...
        elif ((self.current_position[0] != 0 and math.tan(math.radians(self.direction)) == (
                y_lim - self.current_position[1]) / (self.current_position[0])) or (self.direction == 90)):
        ...
        self.current_position[1]) / (x_lim - self.current_position[0])) or (self.direction == 270)):
    ...
    def advertise_connection(self):
        if len(self.bs_data_rate_allocation) == 0:
            ...
            # elif self.last_time >= self.disconnection_time_to_wait:
                ...
                self.last_time = 0
                # print(f"\nim being disconnected {self.ue_id} | last time {self.last_time} | disconnection time to wait {self.connection_time_to_wait}")
                # print(f"lambda_c {self._lambda_c} | lambda_d {self._lambda_d}\n")
                # self.disconnect()
                if self._lambda_c != None:
                    self.connection_time_to_wait = numpy.random.poisson(self._lambda_c)
            else:
    ...
    def connect_(self, bs, rsrp):
        ...
        if len(self.bs_data_rate_allocation) == 0:
            ...
            if current_bs != bs:
                ...
                logging.info("UE %s updated to BS %s with data rate %s --> %s", self.ue_id, current_bs.get_id(),
